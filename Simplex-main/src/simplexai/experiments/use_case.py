...
def mnist_use_case(
    ...
    # FIND COUNTERFACTUAL EXAMPLE 
    test_data = test_inputs[0]
    test_input_label = test_target[test_id:test_id+1]

    # Find potential counterfactuals
    N = corpus_size
    ...
    for i in range(N+1):
        if i<N:
            ...
            xi = torch.flatten(test_data)
            
        for j in range(N+1): 
            if j<N:
                xj = torch.flatten(corpus_inputs[j])
            else:
                xj = torch.flatten(test_data)
    ...
    # Get rid of the 30% longest edges
    distances = np.zeros((N+1))
    new_adj = np.zeros((N+1, N+1))

    for i in range(N+1): 
        distances = adj[i]   
        # find 70th percentile distance
        max_dist_k = np.percentile(distances, 70)
        # discard (edge) distances larger than max_dist_k
    ...
    shortest_paths, predecessors = shortest_path(
        csgraph, method='auto', directed=True, return_predecessors=True, 
    ...
    print("\n \n These are the indices of the path of counterfactuals: ")
    print(path_indices)

    # Extract indices of intermediary example of the path
    if len(path_indices) > 1: 
        intermediary_id = path_indices[len(path_indices//2)]

    
    #### RUN SIMPLEX ON THE COUNTERFACTUAL with usual baseline

    test_datapoint = corpus_inputs[shortest_cft_id]

    print(20 * "-" + "Fitting SimplEx for Counterfactual Analysis" + 20 * "-")
    simplex = Simplex(
        corpus_inputs, classifier.latent_representation(corpus_inputs).detach()
    )
    scheduler = ExponentialScheduler(x_init=0.1, x_final=1000, n_epoch=20000)
    simplex.fit(
        test_datapoint,
        classifier.latent_representation(test_datapoint).detach(),
        n_keep=1,
        n_epoch=20000,
        reg_factor_scheduler=scheduler,
        reg_factor=0.1,
    ...
    output = classifier(test_inputs)
    title = f"Counterfactual: {output.data.max(1, keepdim=True)[1][0].item()}"
    fig = plot_mnist(simplex.test_examples[0][0].cpu().numpy(), title)
    fig.savefig(save_path / f"counterfactual_image_id{test_id}")
    for i in range(1):
        image = decomposition[i][1].cpu().numpy().transpose((1, 2, 0))
        saliency = decomposition[i][2].cpu().numpy().transpose((1, 2, 0))
        title = f"Weight: {decomposition[i][0]:.2g}"
        ...
        fig.savefig(save_path / f"CF_corpus_image{i + 1}_id{test_id}")
    

    #### RUN SIMPLEX ON THE COUNTERFACTUAL with test_input as baseline
    test_datapoint = corpus_inputs[shortest_cft_id]

    print(10 * "-" + "Fitting SimplEx for Counterfactual Analysis, with test_point reference " + 10 * "-")
    simplex = Simplex(
        corpus_inputs, classifier.latent_representation(corpus_inputs).detach()
    )
    scheduler = ExponentialScheduler(x_init=0.1, x_final=1000, n_epoch=20000)
    ...
    print("Changing the input baseline:")
    new_baseline = [test_inputs] * corpus_size
    new_baseline = torch.cat(new_baseline)
    input_baseline = new_baseline
    _ = simplex.jacobian_projection(
        test_id=0, model=classifier, input_baseline=input_baseline, n_bins=200
    )
    decomposition = simplex.decompose(0)
    ...
    output = classifier(test_inputs)
    title = f"Counterfactual, baseline=test_point: {output.data.max(1, keepdim=True)[1][0].item()}"
    fig = plot_mnist(simplex.test_examples[0][0].cpu().numpy(), title)
    fig.savefig(save_path / f"CFbaseline_image_id{test_id}")
    for i in range(1):
        image = decomposition[i][1].cpu().numpy().transpose((1, 2, 0))
        saliency = decomposition[i][2].cpu().numpy().transpose((1, 2, 0))
        title = f"Weight: {decomposition[i][0]:.2g}"
        ...
        fig.savefig(save_path / f"CFbaseline_corpus_image{i + 1}_id{test_id}")
